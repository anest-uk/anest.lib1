...
)
{
  d1 <- min(index(x))
  d2 <- max(index(x))
  mydates <- as.Date(extrca( offda.sdl(x=d1,lags=-max(c(k,0))) ,offda.sdl(x=d2,lags=-min(c(k,0))))) #was aautil::
  res <- lags(as.numeric(coredata(x)),k,pad=TRUE)
  zoo(res,mydates)
}
...
cobalt <- function(light = FALSE) {
  ...
  return(if (light) light_palette else base_palette)
}
...
xcirclefit <-
  function(
      # from package pracma modified: silent, returns rms
      xp,
      yp) {
    if (!is.vector(xp, mode = "numeric") || !is.vector(yp, mode = "numeric")) {
      stop("Arguments 'xp' and 'yp' must be numeric vectors.")
...
#' @export
get_script_dir <-
  function( # from chatgpt to solve the problem of app.R not being runnable
  ) {
    # usage:
    # script_dir <- get_script_dir()
    # data_dir <- file.path(script_dir, "..", "data")
    # saveRDS(obj1, file.path(data_dir, "obj1.rds"))
    this_file <- NULL
    # Rscript
    ...
    if (is.null(this_file)) {
      if (requireNamespace("rstudioapi", quietly = TRUE) &&
        ...
        stop("Can't determine script path — not running in RStudio or with Rscript.")
      }
    }
...
download_pp_csv <- function(destfile = 'pp-complete.csv',
  ...
  if (is.null(destfile)) {
    ...
    destfile <- paste0("pp-complete-", today, ".csv")
  }
  
  tempfile <- paste0(destfile, ".curltmp")
  library(curl)
  ...
  # Monitor file size if in progress (optional check loop)
  elapsed <- 0
  while (file.exists(tempfile) && elapsed < max_wait_sec) {
    Sys.sleep(check_interval_sec)
    size_MB <- round(file.info(tempfile)$size / 1024^2, 1)
    cat(sprintf("Downloaded: %.1f MB\r", size_MB))
    flush.console()
    elapsed <- as.numeric(difftime(Sys.time(), download_started, units = "secs"))
  }
  
  if (file.exists(destfile)) {
    size_MB <- round(file.info(destfile)$size / 1024^2, 1)
    duration <- round(difftime(Sys.time(), download_started, units = "mins"), 1)
    cat(sprintf("\n✅ Done: %s (%.1f MB, %s minutes)\n", destfile, size_MB, duration))
  } else {
